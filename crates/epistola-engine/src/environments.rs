...
pub fn new_environment(collection: &LoadedCollection, name: &str) -> Result<PathBuf, EngineError> {
...
pub fn delete(collection: &LoadedCollection, name: &str) -> Result<(), EngineError> {
...
pub fn get_default(collection: &LoadedCollection) -> Option<String> {
    collection.manifest.default_environment.clone()
}

pub fn set_default(collection: &LoadedCollection, name: &str) -> Result<(), EngineError> {
...
mod tests {
    ...
    fn collection(dir: &Path) -> LoadedCollection {
        LoadedCollection::discover_from(dir).unwrap()
    }
    ...
    fn new_environment_creates_an_empty_environment() {
        ...
        new_environment(&collection(dir.path()), "dev").unwrap();
    ...
    fn set_variable_creates_the_public_file_by_default() {
        ...
        set_variable(&collection(dir.path()), "dev", "k", "v", false).unwrap();
    ...
    fn set_variable_with_secret_writes_the_sidecar() {
        ...
        set_variable(&collection(dir.path()), "dev", "k", "v", true).unwrap();
    ...
    fn list_environment_names_deduplicates_public_and_secrets_files() {
        ...
        set_variable(&collection(dir.path()), "dev", "k", "v", false).unwrap();
        set_variable(&collection(dir.path()), "dev", "k", "v", true).unwrap();
        set_variable(&collection(dir.path()), "prod", "k", "v", false).unwrap();

        let names = list_environment_names(&collection(dir.path())).unwrap();
        assert_eq!(
            names,
    ...
    fn list_environment_names_works_when_no_environments_directory_exists_yet() {
        let dir = tempdir().unwrap();
        CollectionManifest::create(&dir.path().join("epistola.toml"), "n", None).unwrap();
        assert!(list_environment_names(&collection(dir.path()))
            .unwrap()
            .is_empty());
    }
    ...
    fn delete_removes_the_environment() {
        ...
        delete(&collection(dir.path()), "dev").unwrap();
    ...
    fn delete_errors_when_the_environment_is_missing() {
        let dir = tempdir().unwrap();
        CollectionManifest::create(&dir.path().join("epistola.toml"), "n", None).unwrap();
        assert!(delete(&collection(dir.path()), "dev").is_err());
    }
    ...
    fn delete_clears_the_default_environment_if_it_matches() {
        ...
        CollectionManifest::create(&dir.path().join("epistola.toml"), "n", None).unwrap();
        new_environment(&collection(dir.path()), "dev").unwrap();
        set_default(&collection(dir.path()), "dev").unwrap();
    ...
    fn rename_moves_the_environment_and_preserves_variables() {
        ...
        rename(&collection(dir.path()), "dev", "staging").unwrap();
    ...
    fn get_default_is_none_when_unset() {
        let dir = tempdir().unwrap();
        CollectionManifest::create(&dir.path().join("epistola.toml"), "n", None).unwrap();
        assert_eq!(get_default(&collection(dir.path())), None);
    }
    ...
    fn set_default_errors_when_the_named_environment_does_not_exist() {
        let dir = tempdir().unwrap();
        CollectionManifest::create(&dir.path().join("epistola.toml"), "n", None).unwrap();
        assert!(set_default(&collection(dir.path()), "dev").is_err());
    }
